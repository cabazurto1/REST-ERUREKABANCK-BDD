...
export default function RetiroView() {
  ...
  const validarCampos = () => {
    if (!cuenta.trim()) return 'Por favor ingresa un número de cuenta';
    if (cuenta.trim().length !== 8) return 'El número de cuenta debe tener exactamente 8 dígitos';
    const monto = parseFloat(importe);
    if (!importe.trim()) return 'Por favor ingresa un importe';
    if (isNaN(monto) || monto <= 0) return 'El importe debe ser un número mayor que cero';
    return null;
  };
  ...
  const handleRetiro = async () => {
    ...
    try {
      const mensaje = await regRetiro(cuenta.trim(), importe.trim());
      ...
      if (mensaje === '1') {
        ...
        throw new Error('❌ No se pudo completar el retiro.');
      }
    } catch (error) {
      setEstadoTransaccion({
        exito: false,
        mensaje: error.message || '❌ No se pudo completar el retiro.'
      });
    } finally {
      setModalVisible(true);
    }
