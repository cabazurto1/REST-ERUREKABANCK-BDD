...
import { regTransferencia } from '../controllers/OperacionController'; // REST
...
export default function TransferenciaView() {
  ...
  const handleTransferencia = async () => {
    ...
    if (error) {
      setEstadoTransaccion({ exito: 0, mensaje: error });
      setModalVisible(true);
      return;
    }

    try {
      const respuesta = await regTransferencia(origen.trim(), destino.trim(), importe.trim());
      ...
      setEstadoTransaccion({
        exito: 0,
        mensaje: '⚠️ Error de conexión. No se pudo realizar la transferencia.'
      });
    } finally {
      setModalVisible(true);
    }
        ...
        <View style={styles.modalBackground}>
          <View style={styles.modalBox}>
            <Text style={styles.modalTitle}>
              {estadoTransaccion?.exito === 1 ? '✅ Comprobante' : '❌ Error'}
            </Text>
